...
class Cubli {
    private:
        ...
        BLA::Matrix<3,1> accVector = BLA::Zeros<3,1>();
        ...
        void updateStateVector() {
            ...
            // easy mat multiplication for velocity because it is a "true" vector
            velocity = totalTransformActive * velocity; // ground frame

            // annoying three step transform for angles since they are not a "true vector"
            // euler angles are not communiative so their order must be encoded by turning the imu angles
            // into a rot mat and then transforming and finaly taking the product back into euler 
            BLA::Matrix<3,3> rotmIMU = eulerToRotm(angle);
            BLA::Matrix<3,3> rotmIMUActive = totalTransformActive * rotmIMU; 
            angle = rotmToEuler(rotmIMUActive); 
        ...
        void updateOrientation() {
            ...
            accVector = { -mpu.getAccX(), -mpu.getAccY(), -mpu.getAccZ() };
            ...
            accVector = accVector / sqrtf(magAccVectorSquared); // normalized
            ...
            for (auto const& [key, referenceVector] : orientation.crossReferenceTable) {
                float dotProd = (referenceVector * accVector)(0);
    ...
    public:
        ...
        void orientationTest() {
            ...
            if (millis() - lastPrintTime >= 3000) {
                ...
                // DEBUG_PRINTLN("--- RAW ACCEL ---"); 
                // DEBUG_PRINT(mpu.getAccX());DEBUG_PRINT(" | "); 
                // DEBUG_PRINT(mpu.getAccY());DEBUG_PRINT(" | "); 
                // DEBUG_PRINTLN(mpu.getAccZ());

                // DEBUG_PRINTLN("--- Normalized Accleration Values from updateOrientation ---");
                // DEBUG_PRINT(accVector(0,0));DEBUG_PRINT(" | "); 
                // DEBUG_PRINT(accVector(1,0));DEBUG_PRINT(" | "); 
                // DEBUG_PRINTLN(accVector(2,0));
