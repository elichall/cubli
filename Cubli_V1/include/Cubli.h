...
class Cubli {
    private:
        ...
        void updateStateVector() {
            ...
            for (int axis = X; axis <= Z; axis++) {
                ...
                motors[axis].loopFOC(); // phase communication with encoders
    ...
    public:
        ...
        void init() {// --- WAKE SYSTEM UP ---
            ...
            // turn back on the EN pin that was set low for safety
            digitalWrite(PIN_EN, HIGH);
        }
        
        void systemsTest() {// --- GEMINI WRITTEN FUNCTION FOR HARDWARE VALIDATION ---
        ...
        void stateTest() {// --- DEBUG PRINT RELEVANT STATE VARS ---
            ...
            if (millis() - lastPrintTime >= 3000) {
                ...
                // DEBUG_PRINTLN("--- Torque Vector ---");
                // DEBUG_PRINT(torqueVector(X,0));DEBUG_PRINT(" | "); 
                // DEBUG_PRINT(torqueVector(Y,0));DEBUG_PRINT(" | "); 
                // DEBUG_PRINTLN(torqueVector(Z,0));

                DEBUG_PRINTLN("--- Voltage Vector ---");
                DEBUG_PRINT(voltageVector(X,0));DEBUG_PRINT(" | "); 
                DEBUG_PRINT(voltageVector(Y,0));DEBUG_PRINT(" | "); 
                DEBUG_PRINTLN(voltageVector(Z,0));
                DEBUG_PRINTLN(" ");
            }
        }
        ...
        void edgeBalance() {// --- EDGE BALANCE 1D LQR CONTROL LOOP ---
            // safety check for specific edge (need to generalize later)
            ...
            // Get input voltage
            float S = 0.8f; // gain scaler for macro adjustments, also need to tune gain matrix in MatLab
            torqueVector = - (S * gainActive) * stateVector; // ground frame torque 
